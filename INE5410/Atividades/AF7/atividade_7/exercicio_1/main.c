...
int main(int argc, char** argv) {
    ...
    calcular(c, size, 1);
    double duration_1 = omp_get_wtime()-start; //quanto tempo passou
    printf("n_threads: %d, size: %d, tempo: %.3f secs\n", 
           n_threads, size, duration_1);
    ...
    // Calculo de speedup
    double speed_up = duration_1 / duration;
    printf("Speedup de 1 para %d threads é: %f\n", n_threads, speed_up);
