...
void mult_matrix(double* out, double* left, double *right, 
    ...
    int i, j, k;
    int array_length = rows_left * cols_right;
    // Transforma os 3 fors em apenas um, processando de forma "linear" pelas threads,
    // como neste caso os processos tem tamanho constante, não é necessário usar guided
    #pragma omp parallel for schedule(static) collapse(3) reduction(+:out[:array_length])
